<div class="col-md-3">
  ...
  <div class="card card-primary card-outline">
    <div class="card-body box-profile">
      ...
      <button onclick="back_list_user();" type="button" class="btn btn-primary btn-block"><b>Home</b></button>
      ...
      <div class="tab-content">
        ...
        <div class="tab-pane" id="settings">
          <form role="form" id="frmCreateUser"  onsubmit="return add();" name="frmCreateUser" method="POST" action="{{route('users.store')}}" enctype="multipart/form-data"> 
            ...
            <button type="button" class="btn btn-info float-right">Update</button>
          </form>
        </div>
        <div class="tab-pane" id="permission">
            <div class="col-12">
              <div class="form-group">
                <select class="duallistbox" multiple="multiple" id='id_permission'>
                  @foreach($permission as $per)
                      <option  value="{{$per->id}}"
                              @foreach($permission_user as $per_u)
                                  @if($per->id==$per_u->permission_id)
                                      selected
                                    @break
                                  @endif
                              @endforeach
                      >{{$per->description}}</option>
                  @endforeach
                </select>
              </div>
                <button onclick="update_permission('{{$profile_user->id}}')" type="button" class="btn btn-info float-right">Update</button>
              <!-- /.form-group -->
            </div>
...
function update_permission(id_user)
{
  var txt;
  $('#id_permission').each(function() {
    txt = $(this).val();
  });   
  var id_permission ='';
    for (var index = 0; index < txt.length; index++) 
    {
      id_permission=id_permission+txt[index]+',';
      // if(index+1==txt.length)
      // {
      //   id_permission=id_permission+[index];
      // }      
    }

    var _token= $("input[name=_token]").val();
    var hadding='update_permission'
      var url='{{route('users.hadding')}}';//url chuyen huong xu ly usercontroller
      $.ajax({
        headers: { 'X-CSRF-TOKEN': _token},
        url: url,
        data: {hadding:hadding,id_user:id_user,id_permission:id_permission},
        type: 'Post',
        cache:false,      
